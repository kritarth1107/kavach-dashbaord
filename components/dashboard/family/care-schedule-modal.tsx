  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <button
        ...
        onClick={onClose}
      />
      <div className="relative max-h-[90vh] w-full max-w-[520px] overflow-hidden rounded-2xl bg-[var(--card)] shadow-[0_20px_60px_rgba(0,0,0,0.15)]">
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="overflow-y-auto">
          <div className="space-y-3.5 px-5 py-4">
            ...
            {isEdit && (
              <label className="flex cursor-pointer items-center gap-2.5 rounded-lg border border-[var(--border-strong)] bg-[var(--input-bg)] px-3 py-2.5">
                <input
                  type="checkbox"
                  checked={form.active}
                  onChange={(e) => update("active", e.target.checked)}
                  className="h-4 w-4 rounded border-[var(--border-strong)] text-primary focus:ring-primary"
                />
                <span className="text-[12px] font-semibold text-[var(--text-secondary)]">Active on schedule</span>
              </label>
            )}
          </div>
